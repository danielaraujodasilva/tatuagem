...
<body class="bg-gray-950 text-gray-100">
        ...
        <div class="bg-gray-900 border-b border-gray-800 px-8 py-5">
            <div class="flex items-center justify-between">
                ...
                <button onclick="newLead()" 
                        class="bg-emerald-600 hover:bg-emerald-700 px-6 py-3 rounded-xl font-semibold flex items-center gap-2">
                    <i class="fas fa-plus"></i> Novo Lead
                </button>
            </div>

            <!-- Dashboard simples -->
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8" id="dashboard">
                <!-- preenchido via JS -->
            </div>
        </div>
        ...
        <div class="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7 gap-6" id="pipeline">
            <?php foreach($stages as $key => $name): ?>
                <div class="bg-gray-900 rounded-3xl p-5 border border-gray-800" data-stage="<?= $key ?>">
                    ...
                    <div id="total-<?= $key ?>" class="mt-4 text-emerald-400 text-sm font-medium text-right"></div>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div id="modal" class="hidden fixed inset-0 bg-black/80 flex items-center justify-center z-50">
        <div class="bg-gray-900 rounded-3xl w-full max-w-xl mx-4 p-8">
        ...
        function loadPipeline() {
            fetch('handler.php?action=getAll')
                ...
                .then(leads => {
                    ...
                    Object.keys(<?= json_encode(array_keys($stages)) ?>).forEach(k => { counts[k]=0; totals[k]=0; });
                    ...
                    leads.forEach(lead => {
                        ...
                        const cardHTML = `
                            <div class="card bg-gray-800 rounded-3xl p-5 cursor-pointer border border-gray-700" data-id="${lead.id}">
                                ...
                                <div class="mt-4 flex gap-2">
                                    <a href="lead.php?id=${lead.id}" onclick="event.stopImmediatePropagation()" class="text-xs bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded-xl">Detalhes</a>
                                </div>
                            </div>`;
                    ...
                    Object.keys(counts).forEach(k => {
                        ...
                        if (totalEl) totalEl.textContent = totals[k] > 0 ? `Total: R$ ${totals[k].toLocaleString('pt-BR')}` : '';
                    });
        ...
        function updateDashboard(totalLeads, totalValor) {
            const html = `
                <div class="bg-gray-800 rounded-3xl p-6">
                    ...
                    <p class="text-4xl font-bold text-white">${totalLeads}</p>
                </div>
                <div class="bg-gray-800 rounded-3xl p-6">
