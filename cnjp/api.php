...
try {
    ...
    global $config;
    $payload = ['ok' => false, 'message' => 'Erro interno ao carregar o roadmap CNJP.'];
    if (!empty($config['debug'])) $payload['detail'] = $e->getMessage();
...
{
    $stmt = db()->prepare('INSERT INTO cnjp_tasks (task_key, phase, sort_order, title, explanation, question) VALUES (?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE phase=VALUES(phase), sort_order=VALUES(sort_order), title=VALUES(title), explanation=VALUES(explanation), question=VALUES(question)');
    foreach (cnjp_task_definitions() as $task) $stmt->execute($task);
}
...
LEITURA INICIAL
- Núcleo institucional: mediação, arbitragem e atividades auxiliares da justiça.
- Braço tecnológico: TI, treinamento e desenvolvimento de software.
- Tatuagem não entra no posicionamento comercial da CNJP.
- CNAEs paralelos não precisam virar produtos só porque existem.
TXT;
...
function cnjp_bootstrap(array $user): never
{
    ...
    $notesMap=[]; foreach($notes as $note) $notesMap[$note['note_key']]=$note['content'];
...
{
    $input=json_input(); $key=trim((string)($input['task_key']??'')); $status=(string)($input['status']??'todo');
    ...
    $exists=db()->prepare('SELECT id FROM cnjp_tasks WHERE task_key=?'); $exists->execute([$key]);
...
{
    $input=json_input(); $key=preg_replace('/[^a-z0-9_\-]/i','',(string)($input['note_key']??'general'))?:'general'; $content=trim((string)($input['content']??''));
    ...
    $stmt->execute([$key,$content,$_SESSION['user_id']]); audit('update','cnjp_project_note',null,['note_key'=>$key]); json_response(['ok'=>true]);
}

function save_decision(): never
{
    $input=json_input(); $id=(int)($input['id']??0); $title=trim((string)($input['title']??'')); $description=trim((string)($input['description']??'')); $status=(string)($input['decision_status']??'open');
    ...
    if($id>0){$stmt=db()->prepare('UPDATE cnjp_decisions SET title=?, description=?, decision_status=?, updated_at=NOW() WHERE id=?');$stmt->execute([$title,$description,$status,$id]);}
    else{$stmt=db()->prepare('INSERT INTO cnjp_decisions (title,description,decision_status,created_by) VALUES (?,?,?,?)');$stmt->execute([$title,$description,$status,$_SESSION['user_id']]);$id=(int)db()->lastInsertId();}
    audit('update','cnjp_decision',$id,['status'=>$status]); json_response(['ok'=>true,'id'=>$id]);
}

function delete_decision(): never
{
    $id=(int)(json_input()['id']??0); db()->prepare('DELETE FROM cnjp_decisions WHERE id=?')->execute([$id]); audit('delete','cnjp_decision',$id); json_response(['ok'=>true]);
}
