...
try {
    ...
    global $config;
    $p=['ok'=>false,'message'=>'Erro interno ao carregar o roadmap CNJP.'];
    if(!empty($config['debug']))$p['detail']=$e->getMessage();
    json_response($p,500);
}
...
function ensure_cnjp_schema():void{
    ...
    db()->exec("CREATE TABLE IF NOT EXISTS cnjp_ideas (
        id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        title VARCHAR(255) NOT NULL,
        ...
        created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        INDEX idx_cnjp_ideas_created(created_at)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
    ...
    db()->exec("CREATE TABLE IF NOT EXISTS cnjp_history (
        id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
        entity_type ENUM('task','note','decision') NOT NULL,
        entity_key VARCHAR(100) NOT NULL,
        action_type ENUM('create','update','delete','undo') NOT NULL,
        before_json LONGTEXT NULL,
        after_json LONGTEXT NULL,
        user_id INT NULL,
        source_history_id BIGINT UNSIGNED NULL,
        created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        INDEX idx_cnjp_history_created(created_at),
        INDEX idx_cnjp_history_entity(entity_type,entity_key)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
}
...
function seed_cnjp_tasks():void{
    $s=db()->prepare('INSERT INTO cnjp_tasks(task_key,phase,sort_order,title,explanation,question) VALUES(?,?,?,?,?,?) ON DUPLICATE KEY UPDATE phase=VALUES(phase),sort_order=VALUES(sort_order),title=VALUES(title),explanation=VALUES(explanation),question=VALUES(question)');
    foreach(cnjp_task_definitions() as $t)$s->execute($t);
}
...
TXT;
    ...
    $s=db()->prepare("UPDATE cnjp_tasks SET status='done',answer=?,notes=?,updated_by=?,updated_at=NOW() WHERE task_key='company_cnaes' AND (answer IS NULL OR TRIM(answer)='')");
    $s->execute([$answer,$notes,$_SESSION['user_id']??null]);
}
...
function idea_row(int $id):?array{$s=db()->prepare('SELECT id,title,description,created_by,updated_by FROM cnjp_ideas WHERE id=?');$s->execute([$id]);$r=$s->fetch();return $r?:null;}
...
function cnjp_bootstrap(array $user):never{
    ...
    $notes=db()->query('SELECT * FROM cnjp_project_notes')->fetchAll();$nm=[];foreach($notes as $n)$nm[$n['note_key']]=$n['content'];
    $ideas=db()->query('SELECT i.*,cu.name created_by_name,uu.name updated_by_name FROM cnjp_ideas i LEFT JOIN users cu ON cu.id=i.created_by LEFT JOIN users uu ON uu.id=i.updated_by ORDER BY i.id DESC')->fetchAll();
    $dec=db()->query('SELECT d.*,u.name created_by_name FROM cnjp_decisions d LEFT JOIN users u ON u.id=d.created_by ORDER BY d.id DESC')->fetchAll();
    json_response(['ok'=>true,'user'=>$user,'csrf'=>csrf_token(),'tasks'=>$tasks,'notes'=>$nm,'ideas'=>$ideas,'decisions'=>$dec]);
}

function cnjp_history():never{$rows=db()->query("SELECT h.*,u.name user_name FROM cnjp_history h LEFT JOIN users u ON u.id=h.user_id ORDER BY h.id DESC LIMIT 300")->fetchAll();json_response(['ok'=>true,'history'=>$rows]);}

function save_task():never{
    $i=json_input();$k=trim((string)($i['task_key']??''));$st=(string)($i['status']??'todo');if(!in_array($st,['todo','doing','done','blocked'],true))$st='todo';
    $before=row_task($k);if(!$before)json_response(['ok'=>false,'message'=>'Tarefa nao encontrada.'],404);
    $after=['status'=>$st,'answer'=>trim((string)($i['answer']??'')),'notes'=>trim((string)($i['notes']??''))];
    if($before!=$after){$s=db()->prepare('UPDATE cnjp_tasks SET status=?,answer=?,notes=?,updated_by=?,updated_at=NOW() WHERE task_key=?');$s->execute([$after['status'],$after['answer'],$after['notes'],$_SESSION['user_id'],$k]);history_add('task',$k,'update',$before,$after);}
    json_response(['ok'=>true,'saved_at'=>date('c')]);
}

function save_project_note():never{
    $i=json_input();$k=preg_replace('/[^a-z0-9_\-]/i','',(string)($i['note_key']??'general'))?:'general';$content=trim((string)($i['content']??''));
    $s=db()->prepare('SELECT content FROM cnjp_project_notes WHERE note_key=?');$s->execute([$k]);$r=$s->fetch();$before=$r?['content'=>$r['content']]:null;$after=['content'=>$content];
    $st=db()->prepare('INSERT INTO cnjp_project_notes(note_key,content,updated_by) VALUES(?,?,?) ON DUPLICATE KEY UPDATE content=VALUES(content),updated_by=VALUES(updated_by),updated_at=NOW()');$st->execute([$k,$content,$_SESSION['user_id']]);
    if($before!=$after)history_add('note',$k,$before?'update':'create',$before,$after);json_response(['ok'=>true]);
}

function save_idea():never{
    $i=json_input();$id=(int)($i['id']??0);$title=trim((string)($i['title']??''));$desc=trim((string)($i['description']??''));
    if($title==='')json_response(['ok'=>false,'message'=>'Escreva um titulo para a ideia.'],422);
    if($id>0){
        $before=idea_row($id);if(!$before)json_response(['ok'=>false,'message'=>'Ideia nao encontrada.'],404);
        $s=db()->prepare('UPDATE cnjp_ideas SET title=?,description=?,updated_by=?,updated_at=NOW() WHERE id=?');$s->execute([$title,$desc,$_SESSION['user_id'],$id]);
        $after=idea_row($id);if($before!=$after)history_add('note','idea:'.$id,'update',$before,$after);
    }else{
        $s=db()->prepare('INSERT INTO cnjp_ideas(title,description,created_by,updated_by) VALUES(?,?,?,?)');$s->execute([$title,$desc,$_SESSION['user_id'],$_SESSION['user_id']]);$id=(int)db()->lastInsertId();
        $after=idea_row($id);history_add('note','idea:'.$id,'create',null,$after);
    }
    json_response(['ok'=>true,'id'=>$id]);
}

function delete_idea():never{
    $id=(int)(json_input()['id']??0);$before=idea_row($id);if(!$before)json_response(['ok'=>false,'message'=>'Ideia nao encontrada.'],404);
    db()->prepare('DELETE FROM cnjp_ideas WHERE id=?')->execute([$id]);history_add('note','idea:'.$id,'delete',$before,null);json_response(['ok'=>true]);
}

function save_decision():never{
    $i=json_input();$id=(int)($i['id']??0);$title=trim((string)($i['title']??''));$desc=trim((string)($i['description']??''));$status=(string)($i['decision_status']??'open');
    if($title==='')json_response(['ok'=>false,'message'=>'Informe o titulo da decisao.'],422);if(!in_array($status,['open','decided','discarded'],true))$status='open';
    if($id>0){$before=decision_row($id);$s=db()->prepare('UPDATE cnjp_decisions SET title=?,description=?,decision_status=?,updated_at=NOW() WHERE id=?');$s->execute([$title,$desc,$status,$id]);$after=decision_row($id);if($before!=$after)history_add('decision',(string)$id,'update',$before,$after);}
    else{$s=db()->prepare('INSERT INTO cnjp_decisions(title,description,decision_status,created_by) VALUES(?,?,?,?)');$s->execute([$title,$desc,$status,$_SESSION['user_id']]);$id=(int)db()->lastInsertId();history_add('decision',(string)$id,'create',null,decision_row($id));}
    json_response(['ok'=>true,'id'=>$id]);
}
...
function undo_history():never{
    $id=(int)(json_input()['history_id']??0);$s=db()->prepare('SELECT * FROM cnjp_history WHERE id=?');$s->execute([$id]);$h=$s->fetch();if(!$h)json_response(['ok'=>false,'message'=>'Historico nao encontrado.'],404);
    $before=$h['before_json']?json_decode($h['before_json'],true):null;$type=$h['entity_type'];$key=$h['entity_key'];
    if($type==='task'){
        if(!$before)json_response(['ok'=>false,'message'=>'Esta alteracao nao possui estado anterior.'],422);$current=row_task($key);$u=db()->prepare('UPDATE cnjp_tasks SET status=?,answer=?,notes=?,updated_by=?,updated_at=NOW() WHERE task_key=?');$u->execute([$before['status'],$before['answer'],$before['notes'],$_SESSION['user_id'],$key]);history_add('task',$key,'undo',$current,$before,$id);
    }elseif($type==='note' && str_starts_with($key,'idea:')){
        $ideaId=(int)substr($key,5);$current=idea_row($ideaId);
        if($before===null){if($current)db()->prepare('DELETE FROM cnjp_ideas WHERE id=?')->execute([$ideaId]);}
        else{$u=db()->prepare('INSERT INTO cnjp_ideas(id,title,description,created_by,updated_by) VALUES(?,?,?,?,?) ON DUPLICATE KEY UPDATE title=VALUES(title),description=VALUES(description),created_by=VALUES(created_by),updated_by=VALUES(updated_by),updated_at=NOW()');$u->execute([$ideaId,$before['title'],$before['description'],$before['created_by'],$before['updated_by']??$_SESSION['user_id']]);}
        history_add('note',$key,'undo',$current,$before,$id);
    }elseif($type==='note'){
        $q=db()->prepare('SELECT content FROM cnjp_project_notes WHERE note_key=?');$q->execute([$key]);$r=$q->fetch();$current=$r?['content'=>$r['content']]:null;
        if($before===null){db()->prepare('DELETE FROM cnjp_project_notes WHERE note_key=?')->execute([$key]);}
        else{$u=db()->prepare('INSERT INTO cnjp_project_notes(note_key,content,updated_by) VALUES(?,?,?) ON DUPLICATE KEY UPDATE content=VALUES(content),updated_by=VALUES(updated_by),updated_at=NOW()');$u->execute([$key,$before['content'],$_SESSION['user_id']]);}
        history_add('note',$key,'undo',$current,$before,$id);
    }elseif($type==='decision'){
        $did=(int)$key;$current=decision_row($did);
        if($before===null){if($current)db()->prepare('DELETE FROM cnjp_decisions WHERE id=?')->execute([$did]);}
        else{$u=db()->prepare('INSERT INTO cnjp_decisions(id,title,description,decision_status,created_by) VALUES(?,?,?,?,?) ON DUPLICATE KEY UPDATE title=VALUES(title),description=VALUES(description),decision_status=VALUES(decision_status),created_by=VALUES(created_by),updated_at=NOW()');$u->execute([$did,$before['title'],$before['description'],$before['decision_status'],$before['created_by']]);}
        history_add('decision',$key,'undo',$current,$before,$id);
    }
    json_response(['ok'=>true]);
}
