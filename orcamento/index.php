...
function normalizePromoState(source){const input=source&&typeof source==='object'?source:{};return{titulo:String(input.titulo??''),desc:String(input.desc??input.descricao??''),ids:Array.isArray(input.ids)?input.ids:[],desconto:Number(input.desconto??1),view:input.view==='costas'?'costas':'frente',ativa:input.ativa!==false}}
