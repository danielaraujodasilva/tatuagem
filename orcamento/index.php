...
const savedState=<?php echo $orcamentoJson; ?>;
...
function normalizeAreaState(source, fallback){const base=fallback||{};const input=source&&typeof source==='object'?source:{};const price=Number(input.preco??input.min??input.max??base.preco??base.min??base.max??0);return{...base,...input,titulo:String(input.titulo??base.titulo??''),desc:String(input.desc??input.descricao??base.desc??base.descricao??''),preco:Math.round(price/50)*50,min:Math.round(price/50)*50,max:Math.round(price/50)*50,ativa:input.ativa!==false&&base.ativa!==false}}
...
function normalizePromoState(source){const input=source&&typeof source==='object'?source:{};return{titulo:String(input.titulo??''),desc:String(input.desc??input.descricao??''),ids:[...new Set((Array.isArray(input.ids)?input.ids:[]).map(canonicalPromoPieceId).filter(Boolean))],desconto:Number(input.desconto??1),view:input.view==='costas'?'costas':'frente',ativa:input.ativa!==false}}
function applySavedState(state){const input=state&&typeof state==='object'?state:{};if(input.areas&&typeof input.areas==='object'){Object.keys(input.areas).forEach(key=>{if(areas[key])areas[key]=normalizeAreaState(input.areas[key],areas[key])})}if(Array.isArray(input.promos)){promos=input.promos.map(normalizePromoState)}}
...
function calc(){if(!selected.size)return null;const total=[...selected.values()].reduce((sum,i)=>{const d=areas[i.region]||{};return sum+(Number(d.preco??d.min??d.max??0)||0)},0);const p=promos.filter(p=>p.ids.every(id=>selected.has(id))).sort((a,b)=>a.desconto-b.desconto)[0];const disc=p?.desconto||1;return{price:Math.round(total*disc/50)*50,promo:p,off:p?Math.round((1-disc)*100):0,gross:Math.round(total/50)*50}}
function update(){
document.querySelectorAll('.hotspot').forEach(e=>e.classList.toggle('selected',selected.has(e.dataset.id)));
document.querySelectorAll('.region-card').forEach(c=>c.classList.toggle('selected',[...selected.values()].some(i=>i.region===c.dataset.region)));
const items=[...selected.values()],est=calc();
$('count').innerText=items.length;
$('selectionList').innerHTML=items.length?items.map(i=>{const d=areas[i.region]||{};return`<div class="pill"><span>${esc(i.label)}<small>${money(d.preco??d.min??d.max)}</small></span><button data-remove="${i.id}">×</button></div>`}).join(''):'<div style="color:#aaa">Nenhuma região selecionada ainda.</div>';
$('preco').innerText=est?((est.promo?`${money(est.gross)} → ${money(est.price)}`:money(est.price))):'---';
$('descricao').innerText=est?(est.promo?`${est.promo.titulo}: ${est.promo.desc}`:(areas[items.at(-1).region]?.desc||'Região selecionada.')):'Clique em uma ou mais regiões para montar a estimativa inicial.';
$('promoCurrent').style.display=est?.promo?'block':'none';
if(est?.promo)$('promoCurrent').innerHTML=`<b>Promoção aplicada!</b> ${est.off}% OFF · ${money(est.gross)} → ${money(est.price)}`;
renderPromos();links()}
...
function links(){const est=calc();const client=$('cliente').value.trim()||'Cliente ainda não informou';const idea=$('referenciaTexto').value.trim()||'Não descreveu ainda';const areaText=[...selected.values()].map(i=>i.label).join(', ')||'a definir';const price=est?money(est.price):'Aguardando seleção';const msg=['Olá! Quero um orçamento de tatuagem.',`Nome: ${client}`,`Área escolhida: ${areaText}`,`Referência/ideia: ${idea}`,`Pré-orçamento: ${price}`,'Sei que o valor final depende de avaliação.'].join('\n');const href=`https://wa.me/${phone}?text=${encodeURIComponent(msg)}&utm_source=orcamento_3d`;['zap','mobileCta','topZap'].forEach(id=>$(id).href=href)}
