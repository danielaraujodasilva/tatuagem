...
function canonicalPromoPieceId(id){const value=String(id||'');if(value.includes('costas')&&value.includes('esq'))return'costas_esquerda';if(value.includes('costas')&&value.includes('dir'))return'costas_direita';return value}
...
function update(){document.querySelectorAll('.hotspot').forEach(e=>e.classList.toggle('selected',selected.has(e.dataset.id)));document.querySelectorAll('.region-card').forEach(c=>c.classList.toggle('selected',[...selected.values()].some(i=>i.region===c.dataset.region)));const items=[...selected.values()],est=calc();$('count').innerText=items.length;$('selectionList').innerHTML=items.length?items.map(i=>{const d=areas[i.region]||{};return`<div class="pill"><span>${esc(i.label)}<small>${money(d.preco??d.min??d.max)}</small></span><button data-remove="${i.id}">×</button></div>`}).join(''):'<div style="color:#aaa">Nenhuma região selecionada ainda.</div>';$('preco').innerText=est?money(est.price):'---';$('descricao').innerText=est?(est.promo?`${est.promo.titulo}: ${est.promo.desc}`:(areas[items.at(-1).region]?.desc||'Região selecionada.')):'Clique em uma ou mais regiões para montar a estimativa inicial.';$('promoCurrent').style.display=est?.promo?'block':'none';if(est?.promo)$('promoCurrent').innerHTML=`<b>Promoção aplicada!</b> ${est.off}% OFF`;renderPromos();links()}
